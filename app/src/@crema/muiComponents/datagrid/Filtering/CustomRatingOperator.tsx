...
function RatingInputValue(properties: GridFilterInputValueProps) {
  const {item, applyValue, focusElementRef} = properties;

  const ratingReference: React.Ref<any> = React.useRef(null);
  React.useImperativeHandle(focusElementRef, () => ({
    focus: () => {
      ratingReference.current.querySelector(`input[value="${Number(item.value) || ''}"]`).focus();
    },
  }));
