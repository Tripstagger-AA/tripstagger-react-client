...
export default function TransitionGroupExample() {
  ...
  const handleAddFruit = () => {
    const nextHiddenItem = FRUITS.find(index => !fruitsInBasket.includes(index));
    if (nextHiddenItem) {
      setFruitsInBasket(previous => [nextHiddenItem, ...previous]);
    }
  };

  const handleRemoveFruit = (item: string) => {
    setFruitsInBasket(previous => [...previous.filter(index => index !== item)]);
  };
