...
const HorizontalCollapse: React.FC<HorizontalCollapseProperties> = properties => {
  const [opened, setOpened] = useState<boolean>(false);
  const {theme} = useThemeContext();
  const {item, nestedLevel, dense} = properties;
                        ...
                        >
                          {item.children.map(item => (
                            <React.Fragment key={item.id}>
                              {item.type === 'group' && (
                                <HorizontalGroup item={item} nestedLevel={nestedLevel + 1} />
                              )}
                              ...
                              {item.type === 'item' && (
                                <HorizontalItem item={item} nestedLevel={nestedLevel + 1} />
                              )}
                            </React.Fragment>
                          ))}
                        </List>
