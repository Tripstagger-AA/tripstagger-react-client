...
const withWidth = () => (WrappedComponent: any) => (properties: any) =>
  <WrappedComponent {...properties} width='xs' />;

interface AppSemiCircleProgressProperties {
  activeColor: string;
  valueNode: ReactNode;
...
const AppSemiCircleProgress: React.FC<AppSemiCircleProgressProperties> = ({
  activeColor = '#02B732',
  valueNode,
  labelStyle,
  strokeWidth = 10,
  ...
  let rotation;
  if (orientation === 'down') {
    rotation = direction === 'left' ? 'rotate(180deg) rotateY(180deg)' : 'rotate(180deg)';
  } else {
    if (direction === 'right') {
